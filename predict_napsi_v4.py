...
for filename in data['ID'].unique():
    ...
    if os.path.exists(img_path):
        img = load_img(img_path, target_size=(120, 160))  # Ensure correct target size
...
# Define multi-output model architecture
input_img = Input(shape=(120, 160, 3))
...
# Define the model
model = Model(inputs=input_img, outputs=[output_binary, output_napsi])
...
# Save the model
model.save('C:/Users/RO100202/pythonProject/combined_model.h5')


def calculate_napsi_from_binary(binary_predictions):
    # Binary predictions will be reshaped to (8, 4) to represent features and quadrants
    binary_predictions = binary_predictions.reshape((8, 4))

    # Group features as per the rules
    group1_features = binary_predictions[:4]  # First four features
    group2_features = binary_predictions[4:]  # Next four features

    # Calculate scores for each group
    group1_score = np.sum(np.any(group1_features, axis=0))
    group2_score = np.sum(np.any(group2_features, axis=0))
...
new_image_path = 'C:/Users/RO100202/OneDrive - ANRITSU CORPORATION/Desktop/inference/21_nail2.jpg'
pred_df, calculated_napsi_score, raw_napsi_score = predict_napsi_for_image(new_image_path, model)
if pred_df is not None:
    print("Predicted Features:\n", pred_df)
    print("Calculated NAPSI Score:", calculated_napsi_score)
    print("Raw NAPSI Score:", raw_napsi_score)
